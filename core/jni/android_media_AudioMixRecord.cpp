...
// ----------------------------------------------------------------------------
static const char* const kClassPathName = "android/media/AudioMixRecord";
...
static jint android_media_translateRecorderErrorCode(int code) {
    switch(code) {
    case NO_ERROR:
...
static void recorderCallback(int event, void* user, void *info) {
    if (event == AudioMixRecord::EVENT_MORE_DATA) {
        // set size to 0 to signal we're not using the callback to read more data
        AudioMixRecord::Buffer* pBuff = (AudioMixRecord::Buffer*)info;
        pBuff->size = 0;  
    
    } else if (event == AudioMixRecord::EVENT_MARKER) {
    ...
    } else if (event == AudioMixRecord::EVENT_NEW_POS) {
        audiorecord_callback_cookie *callbackInfo = (audiorecord_callback_cookie *)user;
        JNIEnv *env = AndroidRuntime::getJNIEnv();
        if (user && env) {
            env->CallStaticVoidMethod(
        ...
        jint audioFormat, jint buffSizeInBytes, jintArray jSession)
{
    ...
    AudioMixRecord *lpRecorder = NULL;

    // create an uninitialized AudioRecord object
    lpRecorder = new AudioMixRecord();
        if(lpRecorder == NULL) {
    ...
    //     LOGE("Error creating AudioMixRecord instance: initialization check failed.");
...
{
    AudioMixRecord *lpRecorder = 
            (AudioMixRecord *)env->GetIntField(thiz, javaAudioRecordFields.nativeRecorderInJavaObj);
    if (lpRecorder == NULL ) {
...
static jint android_media_AudioRecord_set_marker_pos(JNIEnv *env,  jobject thiz, 
    ...
    AudioMixRecord *lpRecorder = (AudioMixRecord *)env->GetIntField(
...
static jint android_media_AudioRecord_get_min_buff_size(JNIEnv *env,  jobject thiz,
    ...
    int frameCount = 0;
    status_t result = AudioMixRecord::getMinFrameCount(&frameCount,
            sampleRateInHertz,
...
static JNINativeMethod gMethods[] = {
    ...
    {"native_setup",         "(Ljava/lang/Object;IIII[I)I",
...
// ----------------------------------------------------------------------------
int register_android_media_AudioMixRecord(JNIEnv *env)
{
